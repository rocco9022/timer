...
Nuestra App sera un Counter y Countdown. Podremos cambiar entre ambos y escribir una cantidad de segundos para la cuentra regresiva.
...
2- Un estado para llevar el control de nuestro boton de Start y Pausa.
3- Por ultimo necesitaremos un estado para cambiar entre nuestro Counter y nuestra Countdown.
...
...
  return (
    <div className="app">
      ...
      {tipo === 'Countdown' && <input type="number" placeholder="Ingresa Segundos" autoComplete="off"/>}
    </div>
  );
...
```
...
Agregamos la funcionalidad de Start, Pausa y Reseteo del Timer. Primero agregamos una funcion para cambiar nuestro estado `active`. La llamamos 'toggle'. Y cuando esta sea llamada cambiara el state activo.
...
```
Al ejecutarse esta funcion volvemos a nuestros valores iniciales. Por ultimo, una funcion para cambiar entre 'Counter' y 'Countdown'.
...
Para nuestro Counter usaremos el metodo `setInterval`. En lugar de iniciar nuestro Counter en la funcion toggle, usaremos `useEffect` para saber cuando `activo` es true para iniciarlo.
...
{tipo === 'Countdown' && <input type="number" ref={myRef} onChange={agregaSegundos} placeholder="Ingresa Segundos" autoComplete="off"/>}

...
```

Modificamos `useEffect` para agregar la funcionalidad de 'Countdown'. Useremos otro `setInterval` para restar la cantidad de segundos que tengamos en nuestro state. Y otra condicion para resetear nuestro Timer al llegar a 0.
...
...
  useEffect(() => {
    ...
    if (segundos === 0 && activo === 'Countdown') {
      reset();
      clearInterval(intervalo);
    }
